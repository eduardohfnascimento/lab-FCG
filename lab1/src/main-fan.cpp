...
GLuint BuildTriangles()
{
    ...
    NDC_coefficients[0] = 0.0f; // X
    NDC_coefficients[1] = 0.0f; // Y
    NDC_coefficients[2] = 0.0f; // Z
    NDC_coefficients[3] = 1.0f; // W

    float circle = 2 * 3.1415, theta = (22.5 * 3.1415)/180, angle = 0, radius = 0.7;
    int index = 4;
    while(angle <= circle) {
      NDC_coefficients[index] = radius * cos(angle);   // X
      NDC_coefficients[index+1] = radius * sin(angle); // Y
      NDC_coefficients[index+2] = 0.0f;             // Z
      NDC_coefficients[index+3] = 1.0f;             // W
      angle = angle + theta;
      index+=4; // testar acima com i++ direto dentro do []
    };
